...
class ClassesExplorer extends Controller {
    ...
    constructor(parent, cfg = {}) {
        ...
        // classesTabElement is now optional since we're using buttons instead of tabs
        // We'll use a classesButtonElement instead if provided
        this._classesTabElement = cfg.classesTabElement; // For backward compatibility
        this._classesButtonElement = cfg.classesButtonElement; // New parameter for button element

        if (!this._classesTabElement && !this._classesButtonElement) {
            throw "Missing config: classesTabElement or classesButtonElement";
        }
    ...
    setEnabled(enabled) {
        if (!enabled) {
            ...
            if (this._classesButtonElement) {
                this._classesButtonElement.classList.add("disabled");
            } else if (this._classesTabButtonElement) {
            ...
            // Check for the new button element first, then fall back to the old tab button
            if (this._classesButtonElement) {
                this._classesButtonElement.classList.remove("disabled");
            } else if (this._classesTabButtonElement) {
