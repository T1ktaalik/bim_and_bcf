...
class ObjectsExplorer extends Controller {
    ...
    constructor(parent, cfg = {}) {
        ...
        // objectsTabElement is now optional since we're using buttons instead of tabs
        // We'll use a objectsButtonElement instead if provided
        this._objectsTabElement = cfg.objectsTabElement; // For backward compatibility
        this._objectsButtonElement = cfg.objectsButtonElement; // New parameter for button element

        if (!this._objectsTabElement && !this._objectsButtonElement) {
            throw "Missing config: objectsTabElement or objectsButtonElement";
        }
    ...
    setEnabled(enabled) {
        if (!enabled) {
            ...
            if (this._objectsButtonElement) {
                this._objectsButtonElement.classList.add("disabled");
            } else if (this._objectsTabButtonElement) {
            ...
            // Check for the new button element first, then fall back to the old tab button
            if (this._objectsButtonElement) {
                this._objectsButtonElement.classList.remove("disabled");
            } else if (this._objectsTabButtonElement) {
