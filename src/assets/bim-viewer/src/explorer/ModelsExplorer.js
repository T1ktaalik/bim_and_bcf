...
class ModelsExplorer extends Controller {
    ...
    constructor(parent, cfg) {
        ...
        // When models explorer is in a separate element, some elements are optional
        const isInSeparateElement = cfg.isInSeparateElement;

        // modelsTabElement is now optional since we're using buttons instead of tabs
        // We'll use a modelsButtonElement instead if provided
        this._modelsTabElement = cfg.modelsTabElement; // For backward compatibility
        this._modelsButtonElement = cfg.modelsButtonElement; // New parameter for button element

        if (!isInSeparateElement && !this._modelsTabElement && !this._modelsButtonElement) {
            throw "Missing config: modelsTabElement or modelsButtonElement";
        }
    ...
    loadProject(projectId, done, error) {
        this.server.getProject(projectId, (projectInfo) => {
            ...
            if (this._numModelsLoaded < this._numModels) {
                if (this._loadModelsButtonElement) {
            ...
            if (this._enableAddModels) {
                if (this._addModelButtonElement) {
    ...
    unloadProject() {
        ...
        if (this._enableAddModels && this._addModelButtonElement) {
    ...
    setEnabled(enabled) {
        if (!enabled) {
            // Check for the new button element first, then fall back to the old tab button
            if (this._modelsButtonElement) {
                this._modelsButtonElement.classList.add("disabled");
            } else if (this._modelsTabButtonElement) {
            ...
            // Check for the new button element first, then fall back to the old tab button
            if (this._modelsButtonElement) {
                this._modelsButtonElement.classList.remove("disabled");
            } else if (this._modelsTabButtonElement) {
