...
class StoreysExplorer extends Controller {
    ...
    constructor(parent, cfg = {}) {
        ...
        // storeysTabElement is now optional since we're using buttons instead of tabs
        // We'll use a storeysButtonElement instead if provided
        this._storeysTabElement = cfg.storeysTabElement; // For backward compatibility
        this._storeysButtonElement = cfg.storeysButtonElement; // New parameter for button element

        if (!this._storeysTabElement && !this._storeysButtonElement) {
            throw "Missing config: storeysTabElement or storeysButtonElement";
        }
    ...
    setEnabled(enabled) {
        if (!enabled) {
            ...
            if (this._storeysButtonElement) {
                this._storeysButtonElement.classList.add("disabled");
            } else if (this._storeysTabButtonElement) {
            ...
            // Check for the new button element first, then fall back to the old tab button
            if (this._storeysButtonElement) {
                this._storeysButtonElement.classList.remove("disabled");
            } else if (this._storeysTabButtonElement) {
